...
class CenterPredictor(nn.Module, ):
    ...
    def cal_bbox_from_idx(self, idx_2d, size_map, offset_map, return_score=False, return_idx=False):
        """
        idx_2d -  2d index of selected (y,x)
        """
        if idx_2d.dim() == 1:
            idx_y, idx_x = idx_2d
            idx = (idx_y * self.feat_sz + idx_x)[None,None]
            ...
            if return_idx:
                return bbox, torch.tensor([idx_y, idx_x], dtype=torch.long, device=idx_y.device)
            return bbox
        else:
            idx = (idx_2d[:,0] * self.feat_sz + idx_2d[:,1])[None,None,:].expand(-1, 2, -1)
            size = size_map.flatten(2).gather(dim=2, index=idx)
            offset = offset_map.flatten(2).gather(dim=2, index=idx).squeeze(0)

            # bbox = torch.cat([idx_x - size[:, 0] / 2, idx_y - size[:, 1] / 2,
            #                   idx_x + size[:, 0] / 2, idx_y + size[:, 1] / 2], dim=1) / self.feat_sz
            batched_idx_2d = idx_2d.transpose(1,0) # (num_candidate, 2) -> (2, num_candidate)
            # cx, cy, w, h
            bbox = torch.cat([ (batched_idx_2d[1:,:] + offset[:1,...]) / self.feat_sz,
                            (batched_idx_2d[:1,:] + offset[1:,...]) / self.feat_sz,
                            size.squeeze(0)], dim=0).transpose(1,0)
            # if bbox.dim() == 2:
