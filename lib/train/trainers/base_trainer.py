...
class BaseTrainer:
    ...
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, save_epochs=None):
        """
        args:
        ...
        self.save_epochs = save_epochs if save_epochs is not None else [79, 159, 239]
            ...
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if self._checkpoint_dir:
                        if self.settings.local_rank in [-1, 0]:
                            self.save_checkpoint()
                    if  not (epoch-1 in self.save_epochs) and not (epoch > (max_epochs - 1)) and epoch > 1:
                    # if epoch > (max_epochs - 10) or save_every_epoch or epoch % 100 == 0:
                        if self.settings.local_rank in [-1, 0]:
                            self.delete_prev_checkpoint()
            except:
    ...
    def delete_prev_checkpoint(self):
        ...
        file_path = '{}/{}_ep{:04d}.pth.tar'.format(directory, net_type, self.epoch-1)
