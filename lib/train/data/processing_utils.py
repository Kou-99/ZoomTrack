...
def jittered_center_crop_noResize(frames, box_extract, box_gt, search_area_factor, output_sz=None, masks=None):
    """ For each frame in frames, extracts a square crop centered at box_extract, of area search_area_factor^2
    times box_extract area. The extracted crops are then resized to output_sz. Further, the co-ordinates of the box
    box_gt are transformed to the image crop co-ordinates
    ...
    if output_sz is None:
        crop_sz =[torch.tensor(im.shape[:2]) for im in frames_crop]
    else:
        crop_sz = [torch.Tensor([output_sz, output_sz]) for _ in range(num_frames)]

    # find the bb location in the crop
    '''Note that here we use normalized coord'''
    box_crop = [transform_image_to_crop(a_gt, a_ex, rf, c_sz, normalize=True)
                for a_gt, a_ex, rf, c_sz in zip(box_gt, box_extract, resize_factors, crop_sz)]  # (x1,y1,w,h) list of tensors
