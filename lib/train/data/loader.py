...
def ltr_collate_stack1(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        out = None
        numel = 0
        per_batch_numel = None
        stackable = True
        for x in batch:
            x_numel =  x.numel()
            if per_batch_numel is None:
                per_batch_numel = x_numel
            elif x_numel != per_batch_numel:
                stackable = False
                break
            numel += x_numel
        if _check_use_shared_memory() and stackable:
            # If we're in a background process, concatenate directly into a
            # shared memory tensor to avoid an extra copy
            storage = batch[0].storage()._new_shared(numel)
            resize_shape = list(batch[0].size())
            bs = len(batch)
            resize_shape.insert(1, bs)
            out = batch[0].new(storage).view(*resize_shape)  
        if stackable:
            return torch.stack(batch, 1, out=out)
        else:
            return TensorList(batch)
        # if _check_use_shared_memory():
        #     # If we're in a background process, concatenate directly into a
        #     # shared memory tensor to avoid an extra copy
        #     numel = sum([x.numel() for x in batch])
