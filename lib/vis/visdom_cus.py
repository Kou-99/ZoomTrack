...
class VisSearchRegionNoGrid(VisBase):
    def __init__(self, visdom, show_data, title):
        super().__init__(visdom, show_data, title)
        self.count = 0
        self.color = _pascal_color_map()

    def save_data(self, data, **kwargs):
        """
        data - dict contianing
                'image': np.array (h,w,channel)
                'box': torch.tensor (num_box, 4) [0,1)
                'gt_bbox': torch.tensor (4) [0,1)
        """
        image = data['image']
        boxes = torch.cat([data['gt_bbox'].unsqueeze(0), data['box']], dim=0)

        self.raw_data = [image, boxes]

    def draw_data(self):
        disp_image = self.raw_data[0].copy()

        # draw grid
        y, x = disp_image.shape[:-1]
        xyxy = torch.tensor([x,y,x,y])
        ...
        boxes = [b.clone() * xyxy  for b in self.raw_data[1]]

        # draw box and center
        for i, disp_rect in enumerate(boxes):
            # color = ((255 * ((i % 3) > 0)), 255 * ((i + 1) % 2), (255 * (i % 5)) // 4)
            # color = index_to_color(i % 7)
            color = self.color[i+1,:].tolist()
            cv2.rectangle(disp_image,
                          (int(disp_rect[0]), int(disp_rect[1])),
                          (int(disp_rect[0] + disp_rect[2]), int(disp_rect[1] + disp_rect[3])), color, 2)
            if scores is not None:
...
class Visdom:
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            elif mode == 'SearchRegionNoGrid':
                self.registered_blocks[title] = VisSearchRegionNoGrid(self.visdom, show_data, title)
            else:
